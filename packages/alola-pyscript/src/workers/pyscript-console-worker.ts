...
const pyscript = {
  ...
  async run(code: string): Promise<{ state: string; error?: string } | undefined> {
    if (!pyscriptConsole) {
    ...
    for (const line of code.split('\n')) {
      const fut = pyscriptConsole.pyconsole.push(line)
      state = fut.syntax_check
      const wrapped = pyscriptConsole.awaitFut(fut)
      try {
