...
import { IPyscriptConsoleRunner } from '~/types/runner'
import { ConsoleState } from '~/types/console'

interface IUsePyscriptConsoleProps {
  packages?: IPackages
}

export function usePyscriptConsole(props?: IUsePyscriptConsoleProps) {
  ...
  const { packages: globalPackages, timeout } = useContext(PyscriptContext)

  const workerRef = useRef<Worker>()
  const runnerRef = useRef<Remote<IPyscriptConsoleRunner>>()

  const { readFile, writeFile, mkdir, rmdir, watchModules, unwatchModules, watchedModules } =
    useFilesystem({ runner: runnerRef?.current })

  const createWorker = () => {
    const worker = new Worker(new URL('../../workers/pyscript-console-worker', import.meta.url))
  ...
  useEffect(() => {
    if (workerRef.current && !isReady) {
      const init = async () => {
        try {
          setIsLoading(true)
          const runner: Remote<IPyscriptConsoleRunner> = wrap(workerRef.current as Worker)
