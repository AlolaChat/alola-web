...
const hexRegExp = new RegExp('^#?([a-f\\d]{2})([a-f\\d]{2})([a-f\\d]{2})$', 'gi')

module.exports = () => {
  return {
    postcssPlugin: 'postcss-filters',

    Once(root: Root) {
      root.replaceValues(hexRegExp, { fast: `filters(` }, (value) => getFilters(value).filter)
    },
  }
}
